/**
 * Backfill PresetAudiences and PresetThemes by categorizing each Preset's Prompt using Gemini.
 * Run from project root: node scripts/backfill-preset-categories.js
 * Optional: DRY_RUN=1 to log only (no DB writes). Requires GEMINI_API_KEY and DB env.
 */
...
async function main() {
  console.log('Backfill preset categories. DRY_RUN=', DRY_RUN);
  ...
  const [audiences, themes, presets] = await Promise.all([
    models.Audiences.findAll({ order: [['SortOrder', 'ASC'], ['Id', 'ASC']], attributes: ['Id', 'Name'] }),
    models.Themes.findAll({ order: [['SortOrder', 'ASC'], ['Id', 'ASC']], attributes: ['Id', 'Name', 'ParentId'] }),
    models.Presets.findAll({ attributes: ['Id', 'Prompt'] }),
  ]);

  const validAudienceIds = new Set(audiences.map((a) => a.Id));
  const validThemeIds = new Set(themes.map((t) => t.Id));

  console.log('Audiences:', audiences.length, 'Themes:', themes.length, 'Presets:', presets.length);
    ...
    try {
      ...
      await sequelize.transaction(async (t) => {
        await sequelize.query(
          'DELETE FROM [dbo].[PresetAudiences] WHERE PresetId = :presetId',
          { replacements: { presetId: preset.Id }, transaction: t }
        );
        await sequelize.query(
          'DELETE FROM [dbo].[PresetThemes] WHERE PresetId = :presetId',
          { replacements: { presetId: preset.Id }, transaction: t }
        );
      ...
      console.log(`Preset ${preset.Id}: saved ${filteredAudienceIds.length} audiences, ${filteredThemeIds.length} themes.`);
    } catch (err) {
